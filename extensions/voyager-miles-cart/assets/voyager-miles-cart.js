...
class VoyagerMilesCart {
  ...
  async init() {
    ...
    this.settingsApiUrl = 'https://saa-voyager-app-prod.tofmail2022.workers.dev/api/settings/points-rate';
  ...
  async fetchConversionRate() {
    try {
      console.log('[Voyager Cart] Fetching conversion rate from:', this.settingsApiUrl);
      ...
      console.warn('[Voyager Cart] Failed to fetch conversion rate, using default:', this.pointsRate);
    } catch (error) {
  ...
  bindEvents() {
    ...
    const cancelPointsBtn = document.getElementById('voyager-cancel-points-btn');
    if (cancelPointsBtn) {
    ...
    // Tab navigation
    const tabs = document.querySelectorAll('.voyager-tab');
    tabs.forEach(tab => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          ...
          this.showStatus(`Login failed: ${errorData.message || 'Unknown error'}`, 'error');
        }
        return;
      }
      ...
      if (data.success) {
        ...
        document.getElementById('voyager-points-display').style.display = 'block';
  ...
  async fetchCartTotal() {
    ...
    try {
      const subtotalElements = document.querySelectorAll('[class*="subtotal"], [class*="cart-total"], [id*="subtotal"], [id*="cart-total"]');
  ...
  handleZarInput(e) {
    ...
    if (zarValue < minZar || zarValue > maxZar) {
      applyBtn.disabled = true;
      this.showPointsStatus(`Discount amount must be between R${minZar.toFixed(2)} and R${maxZar.toFixed(2)}`, 'error');
      this.updateTotalAfterMiles(0);
    } else if (zarValue > maxAvailableZar) {
      applyBtn.disabled = true;
      this.showPointsStatus(`You can only discount up to R${maxAvailableZar.toFixed(2)} (${this.totalPoints.toLocaleString()} points)`, 'error');
      this.updateTotalAfterMiles(0);
    } else {
  ...
  async handleApplyPoints() {
    ...
    if (!zarAmount || zarAmount <= 0) {
      this.showPointsStatus('Please enter a valid discount amount in ZAR.', 'error');
      return;
    }
    ...
    try {
      ...
      console.log('Member Number:', localStorage.getItem('voyager_member_number'));
    ...
    localStorage.setItem('voyager_remaining_points', remainingPoints.toString());
  ...
  addCartAttributes(pointsUsed, pointsRate) {
    ...
    console.log('Adding cart attributes:', { pointsUsed, pointsRate, sessionId, memberNumber });
    ...
    try {
      ...
      .then(cartState => {
        if (cartState) {
          console.log('[Voyager] Current cart attributes:', cartState.attributes);
        }
      ...
      .catch(error => {
        ...
        this.showPointsStatus('Failed to save points to cart. Please try again.', 'error');
      });
  ...
  showPointsStatus(message, type = 'info') {
    ...
    if (!statusMessage) {
      ...
      this.pointsStatusDiv.insertBefore(statusMessage, this.pointsStatusDiv.firstChild);
    }
    ...
    setTimeout(() => {
      if (statusMessage && statusMessage.parentNode) {
        ...
        const checkoutPrompt = this.pointsStatusDiv.querySelector('.checkout-prompt');
  ...
  clearPointsStatus() {
    if (this.pointsStatusDiv) {
      ...
      const statusMessage = this.pointsStatusDiv.querySelector('.status-message');
      if (statusMessage) {
        statusMessage.style.display = 'none';
      }
      // Only hide the entire div if there's no checkout prompt
  ...
  moveAboveCheckoutButton() {
    ...
    const tryMove = () => {
      ...
      const subtotalSections = document.querySelectorAll('[class*="subtotal"], [class*="cart-total"], [class*="cart-summary"], [id*="subtotal"], [id*="cart-total"]');
      for (const section of subtotalSections) {
        const buttons = section.querySelectorAll('button, a[href*="/checkout"]');
        for (const btn of buttons) {
          ...
          if (text.includes('check out') || text.includes('checkout') || 
            ...
            checkoutButton = btn;
            console.log('[Voyager] Found checkout button near subtotal section:', btn);
            break;
          }
        }
      ...
      if (!checkoutButton) {
        const allButtons = document.querySelectorAll('button, a[href*="/checkout"]');
        for (const btn of allButtons) {
          ...
          if (text.includes('check out') || text.includes('checkout') || 
              ...
              btn.href?.includes('/checkout')) {
            checkoutButton = btn;
            console.log('[Voyager] Found checkout button by text/content:', btn);
            break;
          }
        }
      }
      ...
      if (!checkoutButton) {
        for (const selector of checkoutSelectors) {
          try {
            ...
            if (checkoutButton) {
              console.log('[Voyager] Found checkout button with selector:', selector);
              break;
            }
          } catch (e) {
      ...
      console.log('[Voyager] Checkout button parent:', checkoutButton.parentElement);
      ...
      if (!targetContainer) {
        console.log('[Voyager] ⚠️ Button parent has no parent, using button parent instead');
        // Fallback: use button parent and force full width with CSS
        // Remove block from current position if different
        if (this.block.parentElement && this.block.parentElement !== buttonParent) {
        ...
        try {
          ...
          console.log('[Voyager] ✅ Moved block to button parent with full width');
          return true;
        } catch (error) {
      ...
      console.log('[Voyager] Target container (parent of button parent):', targetContainer);
      console.log('[Voyager] Target container tag:', targetContainer.tagName);
      console.log('[Voyager] Target container class:', targetContainer.className);
      ...
      if (targetContainer.contains(this.block)) {
        ...
        if (blockIndex !== -1 && buttonParentIndex !== -1 && blockIndex < buttonParentIndex) {
          console.log('[Voyager] ✅ Block already in correct position (block index:', blockIndex, ', button parent index:', buttonParentIndex, ')');
          return true;
        }
      }

      // Remove block from current position if it's in a different parent
      if (this.block.parentElement && this.block.parentElement !== targetContainer) {
        this.block.parentElement.removeChild(this.block);
      ...
      try {
        ...
        console.log('[Voyager] ✅ Successfully moved Voyager block above checkout button container');
        return true;
      } catch (error) {
    ...
    setTimeout(() => {
      if (!tryMove()) {
        ...
        setTimeout(() => {
          if (!tryMove()) {
            console.log('[Voyager] ⚠️ Checkout button not found after final retry, keeping default position');
          }
        }, 1000);
      }
    }, 500);
